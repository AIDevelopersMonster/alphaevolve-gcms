...
**Status:** v0.1 technical note / not final manuscript  
**Date:** 2026-05-12  
...
This document is a conservative technical note.

It is not:

```text
a physical theory paper;
a causal mechanism proof;
a density-independent compensation proof;
a universal topology-threshold claim;
a claim about real-world constants, spacetime, gravity, or photons.
```
...
`Compensation-connectivity entanglement` is used only as shorthand for:

```text
observed empirical co-variation between compensation-aware dynamics and higher native topological capacity, without implying directed causality, necessity, universality, or physical interpretation.
```
...
## 5. Reframe and methodological transparency
...
### 7.3 Model-mode rates with Wilson score intervals
...
Unless otherwise stated, rate intervals in this note use 95% Wilson score intervals. The threshold-difference interval uses a bootstrap 95% confidence interval with row-level resampling. These intervals quantify uncertainty in observed descriptive contrasts and do not imply causal identification.
...
This note is based on the following internal artifacts:
